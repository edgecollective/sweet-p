...
#record = sd_ts + "," + str(temperature)+ "," + batt_volts_str + "," + str(depth)+"\n"
print("ts,batt,temp,depth")
...
while True:
    ...
    #print("recording temperature and depth")
        # in case something freezes or we run out of time, first record a failure (send_result=0)
    record = str(the_hour)+":"+str(the_minute)+":"+str(the_second)+","+batt_volts_str+ "," + str(temperature)+ "," + str(depth)+"\n"
    try:
        with open("/sd/freezer5.txt", "a") as f:
            #print("%d %0.1f %d\n" % (index,my_batt,my_depth))
            f.write(record)
            f.close()
    except Exception as error:
    ...
    out_str = str(the_hour)+":"+str(the_minute)+":"+str(the_second)+"\nTemp (C):"+str(temperature)+"\nDepth (cm):"+str(depth)+"\nBatt (V):"+batt_volts_str
    
    text_area.text=out_str
