...
try:
    ...
    # Memory-efficient way to read last line without loading entire file
    with open("/sd/log.txt", "r") as f:
        ...
        if file_size == 0:
            ...
            lines = chunk.split('\n')
            if chunk.endswith('\n') and len(lines) > 1:
                last_line_str = lines[-2]  # Second to last because last is empty
            else:
                last_line_str = lines[-1]  # Last line doesn't end with newline
            
            last_line = last_line_str.strip().split(",")
...
elif (the_hour_mst==5 or the_hour_mst==13 or the_hour_mst==18):
