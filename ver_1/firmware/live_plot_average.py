#!/usr/bin/env python3
"""
Live plotting script for tprobe2.py serial output with fixed depth scale and average depth
Reads voltage, current, depth, and average depth data from serial port and plots in real-time
Depth scale is fixed to 0-1000mm, with both instantaneous and average depth plotted
"""
...
class SerialPlotter:
    def __init__(self, port, baudrate=115200, max_points=100):
        ...
        self.avg_depths = deque(maxlen=max_points)
        ...
        self.fig.suptitle('Live Sensor Data with Average Depth (Fixed Depth Scale)')
        ...
        self.line4, = self.ax3.plot([], [], 'orange', linewidth=2, label='Average Depth (mm)')
    ...
    def read_data(self):
        ...
        try:
            ...
            if line:
                # Parse the space-separated values: voltage current depth avg_depth
                parts = line.split()
                if len(parts) >= 4:
                    voltage = float(parts[0])
                    current = float(parts[1])
                    depth = float(parts[2])
                    avg_depth = float(parts[3])
                    
                    current_time = time.time() - self.start_time
                    
                    return current_time, voltage, current, depth, avg_depth
                elif len(parts) >= 3:
                    # Fallback for old format (3 values)
                    voltage = float(parts[0])
                    current = float(parts[1])
                    depth = float(parts[2])
                    avg_depth = depth  # Use current depth as average if not provided
    ...
    def update_plot(self, frame):
        ...
        if data:
            current_time, voltage, current, depth, avg_depth = data
            ...
            self.line4.set_data(list(self.times), list(self.avg_depths))
            ...
            # Print latest values
            print(f"Time: {current_time:.1f}s, Voltage: {voltage:.2f}mV, Current: {current:.2f}mA, Depth: {depth:.2f}mm, Avg: {avg_depth:.2f}mm")
...
def main():
    parser = argparse.ArgumentParser(description='Live plot serial data with average depth overlay')
