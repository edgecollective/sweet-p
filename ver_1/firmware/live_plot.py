#!/usr/bin/env python3
"""
Live plotting script for tprobe2.py serial output
...
class SerialPlotter:
    def __init__(self, port, baudrate=115200, max_points=100):
        ...
        self.fig.suptitle('Live Sensor Data from tprobe2.py')
    ...
    def update_plot(self, frame):
        ...
        if data:
            ...
            if len(self.times) > 1:
                ...
                if self.depths:
                    depth_range = [min(self.depths) * 0.95, max(self.depths) * 1.05]
                    self.ax3.set_ylim(depth_range)
...
def main():
    parser = argparse.ArgumentParser(description='Live plot serial data from tprobe2.py')
