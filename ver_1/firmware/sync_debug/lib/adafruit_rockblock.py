...
class RockBlock:
    ...
    def _uart_xfer(self, cmd: str) -> Tuple[bytes, ...]:
        ...
        if(cmd == "+SBDIX"):
            ...
            time.sleep(20) # hack because the reply from +SBDIX command is slow to complete
    ...
    def geolocation(
        ...
        if resp[-1].strip().decode() == "OK":
            ...
            # Calculate UTC time by adding seconds to the epoch
    ...
    def system_time(self) -> Optional[time.struct_time]:
        ...
        if resp[-1].strip().decode() == "OK":
            ...
            # DEBUG: Print raw hex value from modem
            print(f"DEBUG ROCKBLOCK: Raw hex from modem: '{temp.strip()}'")

            ticks_since_epoch = int(temp, 16)
            print(f"DEBUG ROCKBLOCK: Ticks since Iridium epoch: {ticks_since_epoch}")
            print(f"DEBUG ROCKBLOCK: Ticks in hex: 0x{ticks_since_epoch:08X}")

            # Convert ticks to seconds (each tick = 90ms)
            # Use integer division to avoid floating point issues
            secs_since_epoch = (ticks_since_epoch * 90) // 1000
            print(f"DEBUG ROCKBLOCK: Seconds since Iridium epoch: {secs_since_epoch}")

            # Iridium ERA3 epoch: February 14, 2025, 18:14:17 UTC
            # ERA3 transition occurred on January 14, 2026 at 18:08:00 UTC
            # The base epoch for ERA3 is February 14, 2025, 18:14:17 UTC
            # (ERA2 was May 11, 2014, 14:23:55 UTC - no longer in use after Jan 14, 2026)
            #
            # Calculate UTC time by adding seconds to the ERA3 epoch directly
            # This avoids mktime/localtime epoch issues in CircuitPython
            result_time = _add_seconds_to_datetime(2025, 2, 14, 18, 14, 17, secs_since_epoch)

            print(f"DEBUG ROCKBLOCK: Calculated UTC time: {result_time.tm_year}-{result_time.tm_mon:02}-{result_time.tm_mday:02} {result_time.tm_hour:02}:{result_time.tm_min:02}:{result_time.tm_sec:02}")
