...
def setup_display(display):
    ...
    try:
        ...
        return wakeup_area, stats_area, time_area
    except Exception as e:
...
def update_display(wakeup_area, stats_area, time_area, rtc, stats):
...
def send_satellite_message(rb, message):
...
def main():
    ...
    # Setup display
    wakeup_area, stats_area, time_area = setup_display(display)
    if not all([wakeup_area, stats_area, time_area]):
    ...
    try:
        ...
        update_display(wakeup_area, stats_area, time_area, rtc, stats)
        ...
        if latest_send_time_index >= 0:
            success = send_satellite_message(rb, "hello world")
