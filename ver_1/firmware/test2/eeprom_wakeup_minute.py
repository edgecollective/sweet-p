...
wakeup_times = [58, 02, 04]  #these are the seconds to wake up
...
while True:
    ...
    try:
        index=wakeup_times.index(t.tm_min)
        print("item #",index)
        print(stats)
        
        # if we got here, then the item is in the list
        if(stats[index]!=t.tm_hour): # then we didn't broadcast for this second of the minute
            print("should broadcast!")
            stats[index]=t.tm_hour #mark that we broadcast this hour
            print("new stats:",stats)
            wakeup_stats_area.text="stats: "+' '.join(str(item) for item in stats)
            write_to_eeprom(stats) # update stats
